...
async function loadTicketDetail(ticketId) {
    ...
    document.getElementById('detailResolved')?.textContent = formatDateTime(ticket.resolved_at);
    document.getElementById('detailClosed')?.textContent = formatDateTime(ticket.closed_at);
...
function initTicketCreateForm() {
    ...
    submitBtn.addEventListener('click', async function (e) {
        ...
        let hasError = false;
        if (!title.value.trim()) { showFieldError('titleError', true); hasError = true; } else { showFieldError('titleError', false); }
        ...
        showToast('Ticket ' + res.data.ticket_number + ' submitted', 'success');
        notifyTicketDataChanged('created');
...
function initTicketActions(session, ticket, ticketId) {
    ...
    if ((session.role === 'ict' || session.role === 'admin') && statusSection) {
        ...
        document.getElementById('updateStatusBtn')?.addEventListener('click', async function () {
            ...
            const res = await API.put('/tickets/update-status.php', { ticket_id: ticketId, status });
