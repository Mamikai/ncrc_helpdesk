...
/* ============================================
   ...
   Admin-only. Body: { asset_tag, category, department,
...
$stmt = $pdo->prepare(
    'INSERT INTO assets (asset_tag, category, department, model, serial_number, status, notes, created_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
);
$stmt->execute([$assetTag, $category, $department, $model, $serialNumber, $status, $notes, $_SESSION['user_id']]);
